    ...
    for i in prange(l, nogil=True, num_threads=cores):
        if link_flows[i] > 0:
            if link_flows[i] > capacity[i]:
                congested_time[i] = link_flows[i] * fftime[i] * alpha[i] * beta[i] * (pow(link_flows[i] / capacity[i], beta[i] - 1)) / capacity[i] + fftime[i] * (1 + alpha[i] * (pow(link_flows[i] / capacity[i], beta[i])))
            else:
...
cpdef void dbpr2_cython(double[:] deltaresult,
    ...
    for i in prange(l, nogil=True, num_threads=cores):
        if link_flows[i] > 0:
            if link_flows[i] > capacity[i]:
                deltaresult[i] = fftime[i] * alpha[i] * beta[i] * (pow(link_flows[i] / capacity[i], beta[i] - 1)) / capacity[i] + link_flows[i] * fftime[i] * alpha[i] * beta[i] * (beta[i] - 1) * (pow(link_flows[i] / capacity[i], beta[i] - 2)) / (capacity[i] * capacity[i]) + fftime[i] * alpha[i] * beta[i] * (pow(link_flows[i] / capacity[i], beta[i] - 1)) / capacity[i]
            else:
